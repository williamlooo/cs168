...
class StudentUSocket(StudentUSocketBase):
    ...
    def connect(self, ip, port):
        ...
        ## Start of Stage 1 ##
        packet = self.new_packet(ack=False, syn=True)

        packet.tcp.seq = self.snd.iss
        #self.log.debug("[DEBUG] packet.tcp.ACK = {0}".format(packet.tcp.ACK))
        self.tx(packet)
        self.state = SYN_SENT
        ## End of Stage 1 ##
    ...
    def rx(self, p):
        ...
        * Send pending ACKs and FINs
        ...
        while not self.rx_queue.empty():
            allowed = self.rcv.nxt
            if self.rx_queue.peek()[1].tcp.seq |GT| allowed:
                self.set_pending_ack()
                break
            else:
                curr = self.rx_queue.pop()[1]
                cut_index = self.rcv.nxt |MINUS| curr.tcp.seq
                self.log.debug("CUT INDEX FOR ACCEPTED PACKET {0}".format(cut_index))
                data = curr.app[cut_index:]
                self.handle_accepted_seg(curr.tcp, data)
    ...
    def update_rto(self, acked_pkt):
        ...
        ## Start of Stage 9 ##
        #only mod rto, srtt, rttvar
        r = self.stack.now - acked_pkt.tx_ts
        self.log.debug("RTT NUMBER IS {0}".format(r))
        ...
        self.log.debug("RTTVAR IS {0}".format(self.rttvar))
        ## End of Stage 9 ##
    ...
    def handle_accepted_payload(self, payload):
        ...
        rcv = self.rcv
        self.log.debug("HANDLING PAYLOAD")
    ...
    def handle_accepted_ack(self, seg):
        ...
        for (ackno, p) in acked_pkts:
            #self.log.debug("IS RETRANS {0}".format(p.retxed))
            if not p.retxed:
                self.update_rto(p)
    ...
    def handle_accepted_fin(self, seg):
        ...
        ## Start of Stage 7 ##
            ...
            if self.fin_ctrl.acks_our_fin(seg.ack):
                ...
                self.state = CLOSING #wow someone else closing too
    ...
    def handle_accepted_seg(self, seg, payload):
        ...
        if not continue_after_ack:
            return
        #self.log.debug("STILL HERE")
        ## Start of Stage 2 ##
        #self.log.debug("IM IN HERE")
        self.log.debug("{0}".format(self.state))
        if self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2) and len(payload) |GT| 0:
            #self.log.debug("INSIDE THIS STATE")
    ...
    def maybe_send(self):
        ...
        flight = snd.nxt | MINUS | snd.una
        while remaining > 0:

            #flight = snd.nxt | MINUS | snd.una |PLUS| bytes_sent

            allowed = snd.wnd |MINUS| bytes_sent |MINUS| flight
            self.log.debug("WINDOW {0}".format(snd.wnd))
            self.log.debug("in flight {0}".format(bytes_sent |PLUS| flight))
            if allowed |LE| 0:
                self.log.debug("WINDOW FULL :( {0}".format(snd.wnd))
                break

            #if allowed |GT| self.mss:
                #allowed = self.mss

            allowed = min(allowed, self.mss, remaining)

            payload = self.tx_data[:allowed]
            p = self.new_packet(data=payload)
            self.tx(p)
            self.tx_data = self.tx_data[allowed:]
    ...
    def check_timer_retx(self):
        ...
        if not self.retx_queue.empty():
            ...
            self.log.debug("RTT {0}".format(time_in_queue))

        ## End of Stage 8 ##

        if time_in_queue >= self.rto and p: #added 'and p'
            self.log.debug("earliest packet seqno={0} rto={1} being rtxed".format(p.tcp.seq, self.rto))
            self.tx(p, retxed=True)

            ## Start of Stage 9 ##
            self.rto = min(2*self.rto, self.MAX_RTO)
            ## End of Stage 9 ##
